...
void test_match_int_literal() {
    Literal lit = match_int_literal("100");
    ...
    // test underscores
    Literal lit = match_int_literal("__1____0____0______");
    assert(lit.kind == IntLit);
    assert(lit.type.Int == 100);

    // these things aren't ints, so we should get InvalidLit
    Literal lit = match_int_literal("-100.0");
    assert(lit.kind == InvalidLit);

    Literal lit = match_int_literal("");
    assert(lit.kind == InvalidLit);

    Literal lit = match_int_literal("aaaa");
    assert(lit.kind == InvalidLit);
}


void test_match_float_literal() {
    Literal lit = match_float_literal("100.0");
    ...
    // test underscores
    Literal lit = match_int_literal("__1____0____0___.0___");
    assert(lit.kind == FloatLit);
    assert(lit.type.Int == 100);
