...
type value =
  ...
  | VFn of (param list) * langtype * expr * ((Lexer.ident * value) list)
...
let rec value_to_string v =
  ...
  | VFn (params, out_type, _, _) ->  (delim_to_str "(" params ", " param_to_string ")") ^ " -> " ^ (langtype_to_string out_type)
...
let rec eval  (env : ((Lexer.ident * value) list)) (expression : expr) : value =
  ...
  | Literal value -> 
    (match value with
     ...
     | Flt f -> VFlt f)
  | Let ((ident, body), next) -> 
    let new_binding = (ident, eval env body) in
    eval (new_binding :: env) next
  | FnDef (params, out_type, body) -> VFn (params, out_type, body, env)
  ...
  | FnCall (fn_name, params_to_eval) -> (
      ...
      | VFn (params, out_type, body, fn_env) -> 
        (
          ...
          let rec_binding = (fn_name, VFn (params, out_type, body, fn_env)) in
